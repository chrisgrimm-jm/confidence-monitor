...
- **YouTube chat** — embeds YouTube's live chat as a Left/Right/**Full screen** panel on the Display (mode dropdown in the card header). Only renders for a **currently live** video, and only when this page is hosted (not `file://`). Control embeds its own separate copy of the same chat (below the chat card's fields) so the producer can scroll back through it independently of what's showing on the Display — useful for catching something missed, like a super chat. Below that, **Highlight for talent**: paste or type a name + message and hit **Send to Talent** to push it onto the Display as its own overlay (9-point placement, adjustable size), until you hit **Hide** or send a new one. (YouTube's chat embed is a cross-origin iframe, so this can't read or click into it directly — it's manual copy-paste by design, not automatic.)
