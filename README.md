...
- **Web / Doc** — mirrors any browser window / monitor onto the talent monitor via screen capture (same mechanism as the program feed). Add sources once: in the Display window click **Capture Window** for each window/monitor you want available; they appear in the **Source** dropdown in Control (labeled by type, e.g. "Window 1", "Monitor 2"). Pick which one the talent sees, then **SHOW** and choose Full / Top band / Bottom band. The producer works in the real window on their own screen — scrolling, clicking, editing a live Google Doc — and the talent sees the pixels in real time. Works with **any** site including the Google Docs editor (no iframe/embedding restrictions); read-only mirror for the talent.
  - A browser page can't enumerate windows itself (privacy), so each source is added via the OS picker once; after that you switch between them from the dropdown.
